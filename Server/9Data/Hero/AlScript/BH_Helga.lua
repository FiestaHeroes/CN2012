...
cExecCheck "Helga_BijouRegen"
	...
	Var.StepFunc = Common_Nothing			-- 헬가의 다음 액션은 아무것도 안하고 대기(침입시 카라시안들이 깨움)
	return ReturnAI.END
end
...
--	----cDebugLog("Helga_Attack")

	-- 헬가에 버핑
	if Var.BuffCharge >= 40 then
		----cDebugLog("HelgaBuff")
		if cTargetHandle(Var.Handle) ~= nil then
			cMessage(Var.Handle, "BH_Helga_Buff")
			cSetAbstate(Var.Handle, HelgaBuffByBijou, 1, 200000000)
		end
		Var.BuffCharge = Var.BuffCharge - 40
	end

	if cTargetHandle(Var.Handle) ~= nil then	-- 타겟이 있으면
...
--	----cDebugLog "Bijou_Main"
	...
	if Var == nil then
		----cDebugLog("Bijou Handle Error:" .. Handle .. ":" .. Var.Handle)
		return
	end
...
cExecCheck "Karasian_Main"
	local Var = MemBlock[Handle]
	if Var == nil then
		----cDebugLog("Karasian Handle Error:" .. Handle .. ":" .. Var.Handle)
		return
	end
